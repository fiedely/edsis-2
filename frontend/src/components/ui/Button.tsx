...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  isLoading?: boolean;
  variant?: 'primary' | 'outline' | 'ghost' | 'danger' | 'glass'; // NEW: Added glass
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, children, isLoading, variant = 'primary', size = 'default', disabled, ...props }, ref) => {
    ...
    const variants = {
      ...
      ghost: "text-gray-500 hover:bg-primary/5 hover:text-primary",
      danger: "bg-red-500 text-white shadow hover:bg-red-600",
      // NEW: Glass Variant (Translucent Black with Blur)
      glass: "bg-black/30 text-white hover:bg-black/50 backdrop-blur-md border border-white/10 shadow-sm",
    };
